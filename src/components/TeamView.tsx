...
export default function TeamView({
  ...
  // Shared state for filters and modals
  ...
  // --- Render: Admin (full control with tabs) ---
  if (isAdmin) {
    return (
      <div className="flex-1 flex flex-col gap-6 animate-fade-in">
        {toast && (
          ...
          <>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden">
              ...
              <div className="border-t border-outline-variant bg-surface-container-lowest px-6 py-4 flex items-center justify-between">
                ...
                {/* Pagination placeholder – you can add real pagination later */}
              </div>
            </div>
          </>
        )}
  ...
  // (This is similar to Accountant view, but we keep it separate for future enhancements)
  return (
    <div className="flex-1 flex flex-col gap-6 animate-fade-in">
      {toast && (
        <div
