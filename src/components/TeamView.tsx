...
import { Plus, Search, Filter, Edit, Ban, CheckCircle, ChevronLeft, ChevronRight, X, Calendar, User, FileText, Check } from 'lucide-react';
...
export default function TeamView({
  ...
  const handleAddEmployeeSubmit = (e: React.FormEvent) => {
    ...
    setIsAddOpen(false);
    showToast(`Profil de ${firstName} ${lastName} créé avec succès.`);
  ...
  const handleApprove = (id: string, name: string) => {
    onApproveLeave(id);
    showToast(`Demande de congé de ${name} approuvée.`);
  };
  ...
  const handleConfirmReject = () => {
    ...
    showToast(`Demande de congé de ${req?.employeeName || 'l\'employé'} refusée.`);
  };
  ...
  // Pending Leave requests for approvals view
  ...
  return (
    <div className="flex-1 flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-h1 font-black text-on-surface tracking-tight md:text-display">
            {userRole === 'admin' ? 'Team Dashboard' : 'Team Directory'}
          </h1>
          <p className="text-body-lg text-on-surface-variant mt-1">
            {userRole === 'admin' 
              ? 'Manage team members, roles, permissions, and leave requests.' 
              : 'View and connect with your colleagues at DIXpertIA.'}
          </p>
        </div>

        {/* Action Button for Admin */}
        {userRole === 'admin' && activeSubTab === 'employees' && (
          <button
      ...
      {/* VIEW A: Employee Management Directory */}
      {(userRole === 'employee' || (userRole === 'admin' && activeSubTab === 'employees')) && (
        <>
          {/* Filters & Search bar matching Screen 5 */}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden">
                  ...
                  {filteredEmployees.length > 0 ? (
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            {member.avatarUrl ? (
                              <img
                                ...
                                className={`w-10 h-10 rounded-full object-cover shrink-0 border border-outline-variant/30 ${
                                  member.status === 'Inactive' ? 'grayscale' : ''
                                }`}
                            ...
                            <div>
                              ...
                              <div className="text-caption text-on-surface-variant md:hidden">{member.email}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-6 text-body-sm text-on-surface-variant font-medium hidden md:table-cell">
                          {member.email}
                        </td>
                        <td className="py-4 px-6">
                        ...
                        {userRole === 'admin' && (
                          <td className="py-4 px-6 text-right">
                            <div className="flex justify-end gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={() => showToast(`Modification du profil de ${member.firstName} (Bientôt disponible)`)}
                              ...
                              <button
                                onClick={() => showToast(`Changement de statut pour ${member.firstName}`)}
                    ...
                    <tr>
                      <td colSpan={5} className="py-12 px-6 text-center text-on-surface-variant font-medium">
                        Aucun employé trouvé avec ces filtres de recherche.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div className="border-t border-outline-variant bg-surface-container-lowest px-6 py-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button className="p-1 border border-outline-variant rounded hover:bg-surface-container transition-colors disabled:opacity-40 cursor-pointer" disabled>
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button className="px-3 py-1 border border-primary bg-primary/5 text-primary rounded text-xs font-bold">1</button>
                <button className="p-1 border border-outline-variant rounded hover:bg-surface-container transition-colors disabled:opacity-40 cursor-pointer" disabled>
      ...
      {/* VIEW B: Leave Approvals (Admin Only) */}
      {userRole === 'admin' && activeSubTab === 'leave-approvals' && (
        <div className="flex flex-col gap-6 animate-fade-in">
          
          {/* Summary Bento Cards matching Screen 7 */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="glass-card rounded-xl p-6 flex items-center gap-4 border border-outline-variant/50 shadow-sm">
              ...
              <div>
                ...
                <div className="text-h1 font-black text-on-surface">{pendingRequests.length + 10}</div>
              </div>
            </div>
            ...
            <div className="glass-card rounded-xl p-6 flex items-center gap-4 border border-outline-variant/50 shadow-sm">
              ...
              <div>
                ...
                <div className="text-h1 font-black text-on-surface">4</div>
              </div>
            </div>
            ...
            <div className="glass-card rounded-xl p-6 flex items-center gap-4 border border-outline-variant/50 shadow-sm">
              ...
              <div>
                <div className="text-[11px] text-on-surface-variant font-bold uppercase tracking-wider mb-1">Employees on Leave</div>
                <div className="text-h1 font-black text-on-surface">8</div>
              </div>
            </div>
      ...
      {/* MODAL 2: Reject Leave Request Modal Overlay */}
      {isRejectOpen && (
