...
def generate_invoice_pdf(invoice_data: dict, output_path: str = None):
    ...
    if items:
        ...
        item_table_data.append(["", "", Paragraph("<b>Subtotal:</b>", label_style), Paragraph(f"${subtotal:.2f}", total_style)])
        item_table_data.append(["", "", Paragraph("<b>Total Due:</b>", label_style), Paragraph(f"${total:.2f}", total_style)])
